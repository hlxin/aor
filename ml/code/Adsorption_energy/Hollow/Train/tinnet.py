...
d_cen_3 = np.array([r['data']['d_cen_3']
                    for r in db.select(purpose='adsorption_energy',
                                       site='hollow')])

d_cen_4 = np.array([r['data']['d_cen_4']
                    for r in db.select(purpose='adsorption_energy',
                                       site='hollow')])
...
width_3 = np.array([r['data']['width_3']
                    for r in db.select(purpose='adsorption_energy',
                                       site='hollow')])

width_4 = np.array([r['data']['width_4']
                    for r in db.select(purpose='adsorption_energy',
                                       site='hollow')])
...
for idx_test in range(0,10):
    for idx_validation in range(0,10):
        ...
        model = Regression(images,
                           ...
                           d_cen_3=d_cen_3, # for TinNet
                           d_cen_4=d_cen_4, # for TinNet
                           width_1=width_1, # for TinNet
                           width_2=width_2, # for TinNet
                           width_3=width_3, # for TinNet
                           width_4=width_4, # for TinNet
